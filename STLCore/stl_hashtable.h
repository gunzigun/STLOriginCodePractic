...
struct __hashtable_iterator {
  ...
  node* cur;                      // 迭代器目前所指向之节点
  hashtable* ht;                  // 保持对容器的连结关系（因为可能需要从bucket，调到bucket）
...
public:
  // bucket个数即 buckets vectors的大小
  size_type bucket_count() const { return buckets.size(); }

  // 总共可以有多少buckets，以下是hash_table的一个member function
  size_type max_bucket_count() const
    { return __stl_prime_list[__stl_num_primes - 1]; }    // 其值为4294967291
  ...
  size_type count(const key_type& key) const
  {
    ...
    // 以下，从bucket list的头开始，一一比对每一个元素的键值，比对成功就累加1
...
// 以下函数判断是否需要重建表格，如果不需要，立刻返回，如果需要，就动手...
template <class V, class K, class HF, class Ex, class Eq, class A>
      ...
      __STL_TRY {
        ...
        for (size_type bucket = 0; bucket < old_n; ++bucket) {
          ...
          while (first) {    // 串行还没结束时
            ...
            // 以下四个操作颇为微妙
            // (1)令就bucket指向其对应节点的下一个节点（以便迭代处理）
...
{
  // 先清除已方的bucket vector，这操作是调用vector::clear，造成所有元素为0
  buckets.clear();
  ...
  __STL_TRY {
    ...
    for (size_type i = 0; i < ht.buckets.size(); ++i) {
      // 复制vector的每一个元素（是个指针，指向hash_table节点）
