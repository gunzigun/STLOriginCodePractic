...
public:
  ...
  // 注意上一行，Iterator定义为RB-tree的const_iterator,这表示set的迭代器无法执行写入操作
  // 这是因为set的元素有一定的次序安排，不允许用户在任意处进行写入操作
  ...
  // 注意：set一定要使用RB-tree的insert_unique()而非insert_equal()
  // multiset才使用RB-tree的insert_equal()
  // 因为set不允许相同键值的存在，multiset才允许相同键值的存在 
  ...
  // 以下所有的set操作行为，RB-tree都已经提供，所以set只要传递调用即可
