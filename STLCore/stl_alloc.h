...
// __STL_STATIC_TEMPLATE_MEMBER_BUG 编译器不支持static members of template classes(模板类静态成员)
...
// 以下是第二级配置器
// 注意，无“template型别参数”，且第二参数完全没有派上用场
...
public:
  ...
  static void * allocate(size_t n)
  {
    // 这是一个指向obj*的指针，即指针的指针
    obj * __VOLATILE * my_free_list;
    obj * __RESTRICT result;

    // 大于配置的上限__MAX_BYTES，就调用第一级配置器
    if (n > (size_t) __MAX_BYTES) {
        return(malloc_alloc::allocate(n));
    }

    // 寻找16个 free lists 中适当的一个，保存指向obj对象的指针
    // 0-15对应不同的obj大小 0-[1-8bytes,都分配8bytes], 1-[9-16bytes,都分配16bytes], 15-[121-128byte,都分配128bytes]
    // free_list指向数组的第一个元素位置，然后my_free_list指向第n个位置
...
#       endif
    ...
    // 调整free list，result指向的空间已经分配出去了，所以my_free_list应该保存，下一个空闲块
...
#       endif /* _NOTHREADS */
    ...
    // 调整free list,回收区块，当前第一个空闲块的地址是*my_free_list，q接上这个
...
__default_alloc_template<threads, inst>::chunk_alloc(size_t size, int& nobjs)
{
    ...
    if (bytes_left >= total_bytes) {
        ...
        // 内存池剩余空间不能完全满足需求量，但是足够供应一个（含）以上的区块
        ...
        if (bytes_left > 0) {
            ...
            *my_free_list = (obj *)start_free;
            ...
            // 这里有点疑惑，这个内存不够啊，可能小于该链上的区块大小，比如 95->96->96
            // 难道size规整了，必定是8的倍数，所以left也是8的倍数？
        }
        ...
        if (0 == start_free) {
            ...
            // 试着检视我们手上拥有的东西，这会造成伤害
            // 我们不打算尝试配置较小的区块，因为那在多进程(multi-process)机器上容易导致灾难
            // 以下搜寻适当的free list
            // 所谓适当，是指 “尚有未用区块，且区块够大” 之free list
            for (i = size; i <= __MAX_BYTES; i += __ALIGN) {
                my_free_list = free_list + FREELIST_INDEX(i);
                p = *my_free_list;
                if (0 != p) {       // 更大的块的 free list 尚有未用块
                    // 调整 free list 以释出未用区块
                    *my_free_list = p -> free_list_link;
                    start_free = (char *)p;
                    end_free = start_free + i;
                    // 递归调用自己，因为有大块释放掉了，现在又有内存了
                    return(chunk_alloc(size, nobjs));
                    // 注意，任何残余零头终将被编入适当的free list中备用
                }
            }
	    end_free = 0;	// 如果出现意外（山穷水尽，到处都没内存可用了）
            // 调用第一级配置，看看out-of-memory机制能否尽点力
            start_free = (char *)malloc_alloc::allocate(bytes_to_get);
            // 这会导致抛出异常（exception），或内存不足的情况获得改善
        }
        heap_size += bytes_to_get;
        end_free = start_free + bytes_to_get;
        // 递归调用自己，因为内存池被扩充了，又有内存了
        return(chunk_alloc(size, nobjs));
    }
}
...
/* We hold the allocation lock.                                         */
/* 返回一个大小为 n 的对象，并且有时候会为适当的free list增加节点 */ 
/* 假设 n 已经适当上调至 8 的倍数 */ 
template <bool threads, int inst>
void* __default_alloc_template<threads, inst>::refill(size_t n)
{
    ...
    // 以下在 chunk 空间内建立 free list
      ...
      *my_free_list = next_obj = (obj *)(chunk + n);    //这时候*my_free_list和next_obj都是返回节点result的下一个
      // 以下将free list 的各节点串接起来
      for (i = 1; ; i++) {  // 从1开始，因为第0个将返回给客户端
        current_obj = next_obj;
        next_obj = (obj *)((char *)next_obj + n);       //这里需要转换成char*，表示地址，不然+n时，会访问错误
        if (nobjs - 1 == i) {
